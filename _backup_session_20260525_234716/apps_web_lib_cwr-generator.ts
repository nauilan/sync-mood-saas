...
export function generateCWR(opts: CWRGeneratorOptions): CWRGeneratorResult {
  ...
  for (const item of obrasFiltradas) {
    const { obra, links, fonogramas, performers } = item
    txCount++
    ...
    for (const pub of controlledPublishers) {
      ...
      lines.push(buildSPU(workSeq, spuIdx, pub.cae ?? '', pub.nome, pub.ipi ?? '0', pub.cae ?? '', role, pub.percentual, pub.percentual))
      recordCount++
      // SPT — Brazil only
      lines.push(buildSPT(workSeq, spuIdx, pub.percentual, pub.percentual))
      recordCount++
      spuIdx++
    }
    ...
    for (const writer of allWriters) {
      ...
      if (writer.controlado || writer.ipi) {
        ...
        lines.push(buildSWR(workSeq, swrIdx, writer.ipi ?? '0', writer.nome.split(' ').slice(-1)[0], writer.nome.split(' ').slice(0, -1).join(' '), writerRole, 'BRA', 'BRA', writer.percentual / 2, writer.percentual / 2))
        recordCount++
        // SWT — Brazil only
        lines.push(buildSWT(workSeq, swrIdx, writer.percentual / 2, writer.percentual / 2))
        recordCount++
