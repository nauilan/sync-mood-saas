...
// GET /api/editoras/[id] — retorna uma editora pelo ID
export async function GET(
  ...
  const res = await fetch(
    `${SUPABASE_URL}/rest/v1/editoras?id=eq.${id}&limit=1`,
    { headers }
  )
  const data = await res.json()
  if (!res.ok) return NextResponse.json({ error: data }, { status: res.status })
  const editora = Array.isArray(data) ? data[0] : null
  if (!editora) return NextResponse.json({ error: 'Editora não encontrada' }, { status: 404 })
