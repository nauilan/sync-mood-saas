...
import {
  ...
  Building2, User, RefreshCw, Music, Disc, Download,
} from 'lucide-react'
...
export default function AutorizacaoDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [saneamentoLoading, setSaneamentoLoading] = useState(false)
  ...
  useEffect(() => {
    async function loadSaneamento() {
      ...
      setSaneamentoLoading(true)
      try {
        const res = await authFetch(`/api/obras/${aut.obra_id}/saneamento`)
        const json = await res.json().catch(() => ({}))
        if (!res.ok) throw new Error((json as any)?.error ?? `Erro ${res.status}`)
        setSaneamento((json as any).data ?? null)
      } catch {
        setSaneamento(null)
      } finally {
        setSaneamentoLoading(false)
      }
    }
  ...
  function handleExportPDF() {
    ...
    const statusIntegridade = saneamento?.integridade?.status ?? '—'
    const pendencias = ((saneamento?.integridade?.pendencias ?? []) as Array<any>)
      .map(p => p?.mensagem ?? p?.codigo ?? 'Pendência')
    const percentualAutorizado = titularesFicha.reduce(
      (total: number, item: any) => total + (Number(item.percentual_brasil) || 0),
      0,
    )
    const parteAutorizada = percentualAutorizado > 0 ? fmtPct(percentualAutorizado) : 'Parte controlada conforme ficha editorial'
    const parteNaoControlada = percentualAutorizado > 0 && percentualAutorizado < 100
      ? fmtPct(100 - percentualAutorizado)
      : '—'
    const tabelaEditorial = titularesFicha.length > 0
      ? `
      <table>
        <tr>
          <td class="lbl">Titular / Função</td>
          <td class="lbl">CPF / CNPJ</td>
          <td class="lbl">Controle BR</td>
          <td class="lbl">Controle EX</td>
          <td class="lbl">Editora Original</td>
          <td class="lbl">Administradora</td>
          <td class="lbl">Data Contrato</td>
        </tr>
        ${titularesFicha.map((item: any) => `
          <tr>
            <td class="val"><strong>${esc(item.titular_nome)}</strong><br/><span class="subval">${esc(item.funcao)}</span></td>
            <td class="val">${esc(item.titular_documento)}</td>
            <td class="val">${esc(fmtPct(item.percentual_brasil))}</td>
            <td class="val">${esc(fmtPct(item.percentual_exterior))}</td>
            <td class="val">${esc(item.editora_original_nome)}<br/><span class="subval">${esc(item.editora_original_cnpj)}</span></td>
            <td class="val">${esc(item.editora_adm_nome)}<br/><span class="subval">${esc(item.editora_adm_cnpj)}</span></td>
            <td class="val">${esc(fmtDate(item.data_contrato))}</td>
          </tr>
        `).join('')}
      </table>`
      : '<p>Nenhum titular controlado encontrado.</p>'
...
<style>
  *{box-sizing:border-box}
  ...
  .section h2{font-size:11px;font-weight:bold;text-transform:uppercase;letter-spacing:.06em;background:#f0f0f0;padding:4px 8px;margin:0;border-radius:2px}
  table{width:100%;border-collapse:collapse;margin-top:0}
  td{padding:5px 8px;border-bottom:1px solid #eee;vertical-align:top}
  .lbl{color:#777;width:42%;font-size:10px;text-transform:uppercase;letter-spacing:.04em}
  .val{color:#111}
  .subval{color:#666;font-size:9px}
  ...
  button{margin-top:16px;padding:8px 20px;background:#6d28d9;color:#fff;border:none;border-radius:6px;cursor:pointer;font-size:12px}
</style>
</head>
<body>
...
<div class="section">
  <h2>Ficha Editorial da Obra Autorizada</h2>
  <table>
    ${row('Título da obra', obraTitulo)}
    ${row('Código interno da obra', saneamento?.obra?.codigo_obra ?? '—')}
    ${row('ISWC', saneamento?.obra?.iswc ?? '—')}
    ${row('Recebedor definido', pagoA)}
    ${row('Parte efetivamente autorizada', parteAutorizada)}
    ${row('Parte não controlada', parteNaoControlada)}
    ${row('Status de integridade da obra', statusIntegridade)}
    ${row('Pendências editoriais', pendencias.length ? pendencias.join(' · ') : '—')}
  </table>
  ${tabelaEditorial}
</div>
...
<div class="section">
  <h2>Limites da Autorização</h2>
  <table>
    ${row('Escopo editorial', 'A presente autorização limita-se exclusivamente à parte controlada pela editora/licenciante.')}
    ${row('Escopo de produto', `Limitada aos fonogramas/videofonogramas/ISRCs informados: ${arr(dp.isrcs).join(', ') || 'não informados'}`)}
  </table>
</div>
...
<button onclick="window.print()">Imprimir / Salvar PDF</button>
  ...
  const titularesFicha = (saneamento?.links ?? [])
    .flatMap((link: any) => (link?.titulares ?? []).map((titular: any) => ({
      link_numero: link?.numero_link,
      titular_nome: titular?.titular?.nome_artistico ?? titular?.titular?.nome_completo ?? '—',
      titular_documento: titular?.titular?.cpf_cnpj ?? '—',
      funcao: titular?.funcao_no_link ?? '—',
      controlado: Boolean(titular?.controlado),
      status_controle: titular?.status_controle ?? '—',
      percentual_brasil: titular?.percentual_controle_brasil,
      percentual_exterior: titular?.percentual_controle_exterior,
      editora_original_nome: titular?.editora_original?.nome_fantasia ?? titular?.editora_original?.nome ?? '—',
      editora_original_cnpj: titular?.editora_original?.cnpj ?? '—',
      editora_adm_nome: titular?.editora_administradora?.nome_fantasia ?? titular?.editora_administradora?.nome ?? '—',
      editora_adm_cnpj: titular?.editora_administradora?.cnpj ?? '—',
      data_contrato: titular?.data_contrato ?? null,
      referencia_documental: titular?.referencia_documental ?? null,
    })))
    .filter((item: any) => item.controlado || item.status_controle === 'controlado')
      ...
      <div className="flex gap-1 border-b border-white/[0.06]">
        {TABS.map(t => (
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
            className={`flex items-center gap-1.5 px-4 py-2.5 text-xs font-medium transition-colors border-b-2 -mb-px ${tab === t.id ? 'border-violet-500 text-violet-300' : 'border-transparent text-white/40 hover:text-white/70'}`}
          >
        ...
        <div className="space-y-6">
              ...
              {arr(dp.isrcs).length > 0 && (
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {arr(dp.isrcs).map((isrc, index) => (
                      <span key={index} className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-white/5 border border-white/[0.06] text-xs text-white/70 font-mono">
          ...
          <section>
            <div className="flex items-center justify-between mb-3 gap-3">
              <p className="text-[10px] font-medium text-white/30 uppercase tracking-wider">Ficha Editorial da Obra</p>
              {saneamentoLoading && (
            ...
            <div className="bg-[#0d1526] border border-white/[0.06] rounded-xl overflow-hidden">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 p-4 border-b border-white/[0.06]">
                <div>
                  <p className="text-[10px] text-white/30 uppercase tracking-wider mb-1">Status editorial</p>
                  <p className="text-sm text-white/80">{saneamento?.integridade?.status ?? '—'}</p>
                </div>
                <div>
                  <p className="text-[10px] text-white/30 uppercase tracking-wider mb-1">Contrato vinculado</p>
                  <p className="text-sm text-white/80">{saneamento?.contrato?.numero ?? saneamento?.contrato?.titulo ?? '—'}</p>
                </div>
                <div>
                  <p className="text-[10px] text-white/30 uppercase tracking-wider mb-1">Código da obra</p>
                  <p className="text-sm text-white/80">{saneamento?.obra?.codigo_obra ?? '—'}</p>
                </div>
              </div>

              {titularesFicha.length === 0 ? (
                <div className="p-4 text-sm text-white/45">
                  Nenhum titular controlado encontrado para compor a ficha editorial.
                </div>
              ) : (
                  ...
                  <table className="min-w-full text-sm">
                    <thead className="bg-white/[0.03]">
                      <tr className="text-left text-[10px] uppercase tracking-wider text-white/35">
                        ...
                        <th className="px-4 py-3 font-medium">Controle BR</th>
                        <th className="px-4 py-3 font-medium">Controle EX</th>
                        <th className="px-4 py-3 font-medium">Editora original</th>
                        <th className="px-4 py-3 font-medium">Administradora</th>
                        <th className="px-4 py-3 font-medium">Data contrato</th>
                      </tr>
                    </thead>
                    <tbody>
                      {titularesFicha.map((item: any, index: number) => (
                        <tr key={`${item.titular_nome}-${index}`} className="border-t border-white/[0.06] text-white/75 align-top">
                          <td className="px-4 py-3 font-mono text-xs text-white/50">{item.link_numero ?? '—'}</td>
                          <td className="px-4 py-3">
                            <div className="font-medium text-white/85">{item.titular_nome}</div>
                            <div className="text-xs text-white/40">{item.status_controle}</div>
                          </td>
                          <td className="px-4 py-3 font-mono text-xs text-white/55">{item.titular_documento}</td>
                          <td className="px-4 py-3">{item.funcao}</td>
                          <td className="px-4 py-3">{fmtPct(item.percentual_brasil)}</td>
                          <td className="px-4 py-3">{fmtPct(item.percentual_exterior)}</td>
                          <td className="px-4 py-3">
                            <div>{item.editora_original_nome}</div>
                            <div className="text-xs text-white/40">{item.editora_original_cnpj}</div>
                          </td>
                          <td className="px-4 py-3">
                            <div>{item.editora_adm_nome}</div>
                            <div className="text-xs text-white/40">{item.editora_adm_cnpj}</div>
                          </td>
                          <td className="px-4 py-3">
                            <div>{fmtDate(item.data_contrato)}</div>
                            {item.referencia_documental && (
                              <div className="text-xs text-white/40 mt-1">{item.referencia_documental}</div>
                            )}
                          </td>
                        </tr>
                      ))}
              ...
              {(saneamento?.integridade?.pendencias ?? []).length > 0 && (
                <div className="p-4 border-t border-white/[0.06]">
                  <p className="text-[10px] text-white/30 uppercase tracking-wider mb-2">Pendências editoriais</p>
                  <div className="flex flex-wrap gap-2">
                    {(saneamento.integridade.pendencias as Array<any>).map((pendencia: any, index: number) => (
                      <span
