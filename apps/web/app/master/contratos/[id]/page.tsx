          ...
          {obrasJson.map((o, i) => (
            <div key={i} className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-4">
              <div className="flex items-start justify-between mb-2">
                ...
                <span className="text-xs text-violet-400 font-semibold">{o.pct_autor}% autor</span>
              </div>
              <div className="flex flex-wrap gap-1.5 mt-2">
                <span className="text-xs bg-violet-500/10 text-violet-400 px-1.5 py-0.5 rounded">
                  {contrato.titular_principal ?? 'Autor'}: {o.pct_autor}% ({o.papel_autor})
                </span>
                {o.co_autores.map((c, ci) => (
                  <span key={ci} className="text-xs bg-sky-500/10 text-sky-400 px-1.5 py-0.5 rounded">
                    {c.nome}: {c.pct}%
                  </span>
                ))}
              </div>
