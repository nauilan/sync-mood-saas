...
import { MOCK_MODELOS_AUTORIZACAO } from '@/lib/mock-config'
import type { ModeloAutorizacaoConfig } from '@/lib/types-config'
import { BookOpen, Edit, Eye, X, Save } from 'lucide-react'

const TIPO_LABEL: Record<string, string> = {
  fonograma: 'Fonograma',
  sincronizacao: 'Sincronização',
  publicidade: 'Publicidade',
  videofonograma: 'Videofonograma',
  incidental: 'Incidental',
}

const TIPO_COLOR: Record<string, string> = {
  fonograma: 'bg-violet-500/10 text-violet-400',
  sincronizacao: 'bg-sky-500/10 text-sky-400',
  publicidade: 'bg-amber-500/10 text-amber-400',
  videofonograma: 'bg-emerald-500/10 text-emerald-400',
  incidental: 'bg-rose-500/10 text-rose-400',
}

export default function ModelosAutorizacaoPage() {
  const [editModalId, setEditModalId] = useState<string | null>(null)
  const [previewId, setPreviewId] = useState<string | null>(null)
  const [editContent, setEditContent] = useState('')
  const [savedToast, setSavedToast] = useState(false)

  function openEdit(modelo: ModeloAutorizacaoConfig) {
    setEditContent(modelo.template_text)
  ...
  const editModelo = MOCK_MODELOS_AUTORIZACAO.find((m) => m.id === editModalId)
  const previewModelo = MOCK_MODELOS_AUTORIZACAO.find((m) => m.id === previewId)

  return (
    <div className="space-y-5">
      <PageHeader
        title="Modelos de Autorização"
        description="Templates de autorizações editáveis para fonogramas, sincronização e outros usos."
      />

      {/* Grid de cards */}
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {MOCK_MODELOS_AUTORIZACAO.map((modelo: ModeloAutorizacaoConfig) => (
          <div
            key={modelo.id}
          ...
          >
              ...
              <div className="flex items-center gap-2 mt-2">
                <span
                  className={`text-[10px] px-2 py-0.5 rounded-md ${
                    TIPO_COLOR[modelo.tipo_autorizacao] ?? 'bg-white/5 text-white/35'
                  }`}
                >
                  {TIPO_LABEL[modelo.tipo_autorizacao] ?? modelo.tipo_autorizacao}
                </span>
            ...
            <p className="text-[10px] text-white/30 line-clamp-2 leading-relaxed">
              {modelo.template_text.substring(0, 120)}...
            </p>

            {/* Variáveis */}
            <div className="flex flex-wrap gap-1">
              {Object.keys(modelo.variaveis_json).map((v) => (
                <span
                  key={v}
                  className="text-[9px] font-mono px-1.5 py-0.5 rounded bg-white/[0.04] text-white/30"
                >
                  {`{{${v}}}`}
                </span>
              ))}
      ...
      {previewId && previewModelo && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-2xl rounded-2xl bg-[#0f0d1a] border border-white/[0.08] shadow-2xl overflow-hidden">
            ...
            <div className="p-5">
              <pre className="w-full bg-white/[0.03] border border-white/[0.06] rounded-xl px-4 py-3 text-xs text-white/55 font-mono whitespace-pre-wrap leading-relaxed max-h-72 overflow-y-auto">
                {previewModelo.template_text}
              </pre>
            </div>
