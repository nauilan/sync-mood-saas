...
export function parseCwr(conteudo: string, _opts?: unknown): CwrArquivo {
  ...
  for (let i = 0; i < linhas.length; i++) {
    ...
    try {
      ...
      // ── SWT (Writer Territory — cotas territoriais de arrecadação) ──────────
      // SWT representa cotas de ARRECADAÇÃO territorial, não percentuais editoriais.
      // Validado: SWR contém os percentuais editoriais corretos (26.67, 24.99, 33.33
      // batem com o sistema externo). SWT tem semântica diferente e NÃO deve
      // sobrescrever os percentuais de obras_links_titulares.
